...
import { REGISTER_OWNER_INPUT_PORT, RegisterOwnerInput, RegisterOwnerInputPort } from '../../../core/application/ports/in/register-owner.input-port';
...
export class RegisterOwnerController implements ExpressController {
  ...
  constructor(@inject(REGISTER_OWNER_INPUT_PORT) private readonly registerOwnerCommandHandler: RegisterOwnerInputPort) {}
  async handler(req: Request<unknown, unknown>, res: Response) {
    ...
    try {
      const command = RegisterOwnerCommand.from({firstName, lastName, ownername, email, phoneNumber, city, description })
