...
import { ImpulseAlreadyExists, ImpulseSettingNotFound, MissingImpulseGraphDataFound } from '../../../core/domain/impulse/errors';
...
export class RegisterImpulseController implements ExpressController {
  ...
  constructor(
    @inject(REGISTER_IMPULSE_INPUT_PORT) private readonly commandHandler: RegisterImpulseInputPort,
  ) {}
  async handler(req: Request<unknown, unknown>, res: Response) {
    const { ownerUid, cabinetUid, driverUid, measurements } = req.body;
    try {
      const command = RegisterImpulseCommand.from({ ownerUid, cabinetUid, driverUid, measurements })
