...
import { ImpedanceAlreadyExists, UnableToExtractImpedanceData } from '../../../core/domain/impedance/errors';
...
export class RegisterImpedanceController implements ExpressController {
  ...
  constructor(
    @inject(REGISTER_IMPEDANCE_INPUT_PORT) private readonly commandHandler: RegisterImpedanceInputPort,
  ) {}
  async handler(req: Request<unknown, unknown>, res: Response) {
    ...
    try {
      const command = RegisterImpedanceCommand.from({ ownerUid, cabinetUid, driverUid, measurements })
      ...
      if (error instanceof UnableToExtractImpedanceData) throw new httpErrors.NotFound(error.message);
      throw error;
    }
  }
}
