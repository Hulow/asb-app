...
import { MeasurementQueryMapperOutputPort } from '../../../core/application/ports/out/measurement-query.mapper.output-port';
...
export enum ImpedanceColumnNames {
  ...
  XI = 'xi',
  CabinetUid = 'cabinet_uid',
  ImpedanceCurve = 'impedance_curve',
  CreatedAt = 'created_at',
  UpdatedAt = 'updated_at',
}
...
@injectable()
export class MeasurementQueryMapper implements MeasurementQueryMapperOutputPort {
  ...
  private _generateTables(): TableAndColumNames[] {
    return [
      ...
      { columns: Object.values(FrequencyColumnNames), name: EntityTable.Frequency },
      { columns: Object.values(ImpedanceColumnNames), name: EntityTable.Impedance },
    ];
  }
  ...
  private _mapDriverQuery(key: string, column: DriverColumnNames, tableName: string): string {
    return `', ${key}: ' || ${tableName}.${column} ||`;
  }

  _generatedMappedDriver(): { key: string; column: DriverColumnNames; name: EntityTable }[] {
    const driverTable = {
    ...
    const mappedDriverTable: { key: string; column: DriverColumnNames; name: EntityTable }[] = [];
    let counter = 0;
    for (const key of driverTable.keys) {
      mappedDriverTable.push({ key: key, column: driverTable.columns[counter], name: EntityTable.Driver });
      counter++;
    }
