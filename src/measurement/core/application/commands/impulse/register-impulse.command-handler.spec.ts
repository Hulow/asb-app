...
import { ImpulseAlreadyExists, ImpulseSettingNotFound, MissingImpulseGraphDataFound } from '../../../domain/impulse/errors';
...
describe('Given a RegisterImpulseCommand to handle', () => {
    ...
    const VALID_MEASUREMENT = fs.readFileSync(path.join(__dirname, './inputs/impulse_response.txt'), 'utf8')
    const UNVALID_SETTING_MEASUREMENT = fs.readFileSync(path.join(__dirname, './inputs/impulse_response_with_missing_setting.txt'), 'utf8')
    const UNVALID_DATA_MEASUREMENT = fs.readFileSync(path.join(__dirname, './inputs/wrong_impulse_response.txt'), 'utf8');
    ...
    const startHandler = () => {
        handler = new RegisterImpulseCommandHandler(impulseRepository, cabinetRepository);
    }
