...
export class RegisterImpulseCommandHandler implements RegisterImpulseInputPort {
  constructor(
    @inject(IMPULSE_REPOSITORY_OUTPUT_PORT)
    private readonly impulseRepository: ImpulseRepositoryOutputPort,
  ...
  async execute(command: RegisterImpulseCommand): Promise<Impulse> {
    ...
    const existingImpulse = await this.impulseRepository.getByCabinetUid(
      command.cabinetUid,
    )
    if (existingImpulse) {
    ...
    const newImpulse: ImpulseProps = new RegisterImpulseMapper().mapImpulse(
      command,
    )
  ...
  private verifyResponseLength(newImpulse: ImpulseProps) {
    if (newImpulse.measurements.length !== Number(newImpulse.responseLength)) {
      throw new MissingImpulseGraphDataFound(
        newImpulse.responseLength,
        newImpulse.measurements.length,
      )
    }
  }
}
