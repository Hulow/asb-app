...
export class RegisterImpulseMapper {
  ...
  private mapSingleSetting(
    ...
    for (const setting of measurementSettings) {
      const matchedSetting = this.matchSetting(
        setting,
        settingNameAndPattern.pattern,
      )
      if (matchedSetting) {
        return {
          name: settingNameAndPattern.name,
          value: matchedSetting.trim(),
        }
      }
    }
  ...
  private getImpulseGraphTimeLine(
    impulseGraphData: string[],
    sampleInterval: number,
    startTime: number,
  ) {
    const impulseGraphTimeLine: number[] = []
    impulseGraphTimeLine.push(startTime)
    impulseGraphData.map(() =>
      this.addNewTimeEntry(impulseGraphTimeLine, sampleInterval),
    )
    return impulseGraphTimeLine
  }

  private addNewTimeEntry(
    impulseGraphTimeLine: number[],
    sampleInterval: number,
  ) {
    const lastTimeEntry: number =
      impulseGraphTimeLine[impulseGraphTimeLine.length - 1]
    const newTimeEntry = lastTimeEntry + sampleInterval
    return impulseGraphTimeLine.push(newTimeEntry)
  }

  private mapTimeAndDecibel(
    impulseGraphTimeLine: number[],
    impulseGraphData: string[],
  ): ImpulseGraph[] {
    let counter = 0
    ...
    for (const data of impulseGraphData) {
      const impulse: ImpulseGraph = {
        dbfs: Number(data),
        time: impulseGraphTimeLine[counter],
      }
      ++counter
  ...
  private mapImpulseObject(
    ...
    return {
      ...
      peakValueBeforeInitialization:
        mappedMeasurementSettings.peakValueBeforeInitialization,
...
const settingPaterns = {
  ...
  note: /(?<=Note: ).*/,
  peakValueBeforeInitialization:
    /[\s\S]*?(?= \/\/ Peak value before normalisation)/,
...
const SETTING_NAME_AND_PATTERNS: SettingNameAndPattern[] = [
  ...
  {
    name: SettingKeyNames.ResponseWindow,
    pattern: settingPaterns.responseWindow,
  },
  { name: SettingKeyNames.Note, pattern: settingPaterns.note },
  {
    name: SettingKeyNames.PeakValueBeforeInitialization,
    pattern: settingPaterns.peakValueBeforeInitialization,
  },
  { name: SettingKeyNames.PeakIndex, pattern: settingPaterns.peakIndex },
  {
    name: SettingKeyNames.ResponseLength,
    pattern: settingPaterns.responseLength,
  },
  {
    name: SettingKeyNames.SampleInterval,
    pattern: settingPaterns.sampleInterval,
  },
