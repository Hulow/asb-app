...
import { ImpedanceAlreadyExists, ImpedanceParameterNotFound } from '../../../domain/impedance/errors';
...
describe('Given a RegisterImpedanceCommand to handle', () => {
    ...
    const VALID_MEASUREMENT = fs.readFileSync(path.join(__dirname, './inputs/valid_impedance_response.txt'), 'utf8')
    const UNVALID_TSP_MEASUREMENT = fs.readFileSync(path.join(__dirname, './inputs/unvalid_tsp_impedance_response.txt'), 'utf8')
    ...
    const startHandler = () => {
        handler = new RegisterImpedanceCommandHandler(impedanceRepository, cabinetRepository);
    }
