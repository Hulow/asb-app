...
import { IMPEDANCE_REPOSITORY_OUTPUT_PORT, ImpedanceRepositoryOutputPort } from '../../ports/out/impedance-repository.output-port';
...
@injectable()
export class RegisterImpedanceCommandHandler implements RegisterImpedanceInputPort {
  constructor(
  ...
  async execute(command: RegisterImpedanceCommand): Promise<Impedance> {
    ...
    const existingImpedance = await this.impedanceRepository.getByCabinetUid(command.cabinetUid);
